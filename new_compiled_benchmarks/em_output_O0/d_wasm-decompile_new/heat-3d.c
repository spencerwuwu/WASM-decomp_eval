

extern int64_t stack_pointer;
extern int64_t memory_base;
extern int64_t table_base;
extern int64_t stderr;

data ()rodata(offset memory_base) =
"begin ()dump %s\00A\00%0.2lf \00\0aend ()dump %s\0a\00==BEGIN DUMP_ARR"
"AYS==\0a\00==END DUMP_ARRAYS==\0a\00";









void wasm_call_ctors() {
}

void wasm_apply_data_relocs() {
}

void kernel_heat_3d(int a, int b, int c, int d) {
int64_t* e = stack_pointer;
int64_t f = 32;
{ g (int,)a (int,)b (int,)c (int,)d (int,)e (int,)f (int,)g (int64_t)h } =
e - f;
g.h = a;
g.g = b;
g.f = c;
g.e = d;
int64_t h = 1;
g.d = h;
while (1) {
int64_t i = g.d;
int64_t j = 500;
int64_t k = i;
int64_t l = j;
int64_t m = k <= l;
int64_t n = 1;
int64_t o = m & n;
if (eqz(o)) goto B_a;
int64_t p = 1;
g.c = p;
while (1) {
int64_t q = g.c;
int64_t r = g.g;
int64_t s = 1;
int64_t t = r - s;
int64_t u = q;
int64_t v = t;
int64_t w = u < v;
int64_t x = 1;
int64_t y = w & x;
if (eqz(y)) goto B_c;
int64_t z = 1;
g.b = z;
while (1) {
int64_t aa = g.b;
int64_t ba = g.g;
int64_t ca = 1;
int64_t da = ba - ca;
int64_t ea = aa;
int64_t fa = da;
int64_t ga = ea < fa;
int64_t ha = 1;
int64_t ia = ga & ha;
if (eqz(ia)) goto B_e;
int64_t ja = 1;
g.a = ja;
while (1) {
int64_t ka = g.a;
int64_t la = g.g;
int64_t ma = 1;
int64_t na = la - ma;
int64_t oa = ka;
int64_t pa = na;
int64_t qa = oa < pa;
int64_t ra = 1;
int64_t sa = qa & ra;
if (eqz(sa)) goto B_g;
int64_t ta = g.f;
int64_t ua = g.c;
int64_t va = 1;
int64_t wa = ua + va;
int64_t xa = 115200;
int64_t ya = wa * xa;
int64_t za = ta + ya;
int64_t ab = g.b;
int64_t bb = 960;
int64_t cb = ab * bb;
int64_t db = za + cb;
int64_t eb = g.a;
int64_t fb = 3;
int64_t gb = eb << fb;
int64_t * hb = db + gb;
double ib = hb[0];
int64_t jb = g.f;
int64_t kb = g.c;
int64_t lb = 115200;
int64_t mb = kb * lb;
int64_t nb = jb + mb;
int64_t ob = g.b;
int64_t pb = 960;
int64_t qb = ob * pb;
int64_t rb = nb + qb;
int64_t sb = g.a;
int64_t tb = 3;
int64_t ub = sb << tb;
int64_t * vb = rb + ub;
double wb = vb[0];
double xb = wb + wb;
double yb = ib - xb;
int64_t zb = g.f;
int64_t ac = g.c;
int64_t bc = 1;
int64_t cc = ac - bc;
int64_t dc = 115200;
int64_t ec = cc * dc;
int64_t fc = zb + ec;
int64_t gc = g.b;
int64_t hc = 960;
int64_t ic = gc * hc;
int64_t jc = fc + ic;
int64_t kc = g.a;
int64_t lc = 3;
int64_t mc = kc << lc;
int64_t * nc = jc + mc;
double oc = nc[0];
double pc = yb + oc;
int64_t qc = g.f;
int64_t rc = g.c;
int64_t sc = 115200;
int64_t tc = rc * sc;
int64_t uc = qc + tc;
int64_t vc = g.b;
int64_t wc = 1;
int64_t xc = vc + wc;
int64_t yc = 960;
int64_t zc = xc * yc;
int64_t ad = uc + zc;
int64_t bd = g.a;
int64_t cd = 3;
int64_t dd = bd << cd;
int64_t * ed = ad + dd;
double fd = ed[0];
int64_t gd = g.f;
int64_t hd = g.c;
int64_t id = 115200;
int64_t jd = hd * id;
int64_t kd = gd + jd;
int64_t ld = g.b;
int64_t md = 960;
int64_t nd = ld * md;
int64_t od = kd + nd;
int64_t pd = g.a;
int64_t qd = 3;
int64_t rd = pd << qd;
int64_t * sd = od + rd;
double td = sd[0];
double ud = td + td;
double vd = fd - ud;
int64_t wd = g.f;
int64_t xd = g.c;
int64_t yd = 115200;
int64_t zd = xd * yd;
int64_t ae = wd + zd;
int64_t be = g.b;
int64_t ce = 1;
int64_t de = be - ce;
int64_t ee = 960;
int64_t fe = de * ee;
int64_t ge = ae + fe;
int64_t he = g.a;
int64_t ie = 3;
int64_t je = he << ie;
int64_t * ke = ge + je;
double le = ke[0];
double me = vd + le;
double ne = 0.125;
double oe = ne * me;
double pe = 0.125;
double qe = pc * pe;
double re = qe + oe;
int64_t se = g.f;
int64_t te = g.c;
int64_t ue = 115200;
int64_t ve = te * ue;
int64_t we = se + ve;
int64_t xe = g.b;
int64_t ye = 960;
int64_t ze = xe * ye;
int64_t af = we + ze;
int64_t bf = g.a;
int64_t cf = 1;
int64_t df = bf + cf;
int64_t ef = 3;
int64_t ff = df << ef;
int64_t * gf = af + ff;
double hf = gf[0];
int64_t _if = g.f;
int64_t jf = g.c;
int64_t kf = 115200;
int64_t lf = jf * kf;
int64_t mf = _if + lf;
int64_t nf = g.b;
int64_t of = 960;
int64_t pf = nf * of;
int64_t qf = mf + pf;
int64_t rf = g.a;
int64_t sf = 3;
int64_t tf = rf << sf;
int64_t * uf = qf + tf;
double vf = uf[0];
double wf = vf + vf;
double xf = hf - wf;
int64_t yf = g.f;
int64_t zf = g.c;
int64_t ag = 115200;
int64_t bg = zf * ag;
int64_t cg = yf + bg;
int64_t dg = g.b;
int64_t eg = 960;
int64_t fg = dg * eg;
int64_t gg = cg + fg;
int64_t hg = g.a;
int64_t ig = 1;
int64_t jg = hg - ig;
int64_t kg = 3;
int64_t lg = jg << kg;
int64_t * mg = gg + lg;
double ng = mg[0];
double og = xf + ng;
double pg = 0.125;
double qg = og * pg;
double rg = qg + re;
int64_t sg = g.f;
int64_t tg = g.c;
int64_t ug = 115200;
int64_t vg = tg * ug;
int64_t wg = sg + vg;
int64_t xg = g.b;
int64_t yg = 960;
int64_t zg = xg * yg;
int64_t ah = wg + zg;
int64_t bh = g.a;
int64_t ch = 3;
int64_t dh = bh << ch;
int64_t * eh = ah + dh;
double fh = eh[0];
double gh = rg + fh;
int64_t hh = g.e;
int64_t ih = g.c;
int64_t jh = 115200;
int64_t kh = ih * jh;
int64_t lh = hh + kh;
int64_t mh = g.b;
int64_t nh = 960;
int64_t oh = mh * nh;
int64_t ph = lh + oh;
int64_t qh = g.a;
int64_t rh = 3;
int64_t sh = qh << rh;
int64_t * th = ph + sh;
th[0] = gh;
int64_t uh = g.a;
int64_t vh = 1;
int64_t wh = uh + vh;
g.a = wh;
//continue L_h;
}
//unreachable;
B_g:;
int64_t xh = g.b;
int64_t yh = 1;
int64_t zh = xh + yh;
g.b = zh;
//continue L_f;
}
//unreachable;
B_e:;
int64_t ai = g.c;
int64_t bi = 1;
int64_t ci = ai + bi;
g.c = ci;
//continue L_d;
}
//unreachable;
B_c:;
int64_t di = 1;
g.c = di;
while (1) {
int64_t ei = g.c;
int64_t fi = g.g;
int64_t gi = 1;
int64_t hi = fi - gi;
int64_t ii = ei;
int64_t ji = hi;
int64_t ki = ii < ji;
int64_t li = 1;
int64_t mi = ki & li;
if (eqz(mi)) goto B_i;
int64_t ni = 1;
g.b = ni;
while (1) {
int64_t oi = g.b;
int64_t pi = g.g;
int64_t qi = 1;
int64_t ri = pi - qi;
int64_t si = oi;
int64_t ti = ri;
int64_t ui = si < ti;
int64_t vi = 1;
int64_t wi = ui & vi;
if (eqz(wi)) goto B_k;
int64_t xi = 1;
g.a = xi;
while (1) {
int64_t yi = g.a;
int64_t zi = g.g;
int64_t aj = 1;
int64_t bj = zi - aj;
int64_t cj = yi;
int64_t dj = bj;
int64_t ej = cj < dj;
int64_t fj = 1;
int64_t gj = ej & fj;
if (eqz(gj)) goto B_m;
int64_t hj = g.e;
int64_t ij = g.c;
int64_t jj = 1;
int64_t kj = ij + jj;
int64_t lj = 115200;
int64_t mj = kj * lj;
int64_t nj = hj + mj;
int64_t oj = g.b;
int64_t pj = 960;
int64_t qj = oj * pj;
int64_t rj = nj + qj;
int64_t sj = g.a;
int64_t tj = 3;
int64_t uj = sj << tj;
int64_t * vj = rj + uj;
double wj = vj[0];
int64_t xj = g.e;
int64_t yj = g.c;
int64_t zj = 115200;
int64_t ak = yj * zj;
int64_t bk = xj + ak;
int64_t ck = g.b;
int64_t dk = 960;
int64_t ek = ck * dk;
int64_t fk = bk + ek;
int64_t gk = g.a;
int64_t hk = 3;
int64_t ik = gk << hk;
int64_t * jk = fk + ik;
double kk = jk[0];
double lk = kk + kk;
double mk = wj - lk;
int64_t nk = g.e;
int64_t ok = g.c;
int64_t pk = 1;
int64_t qk = ok - pk;
int64_t rk = 115200;
int64_t sk = qk * rk;
int64_t tk = nk + sk;
int64_t uk = g.b;
int64_t vk = 960;
int64_t wk = uk * vk;
int64_t xk = tk + wk;
int64_t yk = g.a;
int64_t zk = 3;
int64_t al = yk << zk;
int64_t * bl = xk + al;
double cl = bl[0];
double dl = mk + cl;
int64_t el = g.e;
int64_t fl = g.c;
int64_t gl = 115200;
int64_t hl = fl * gl;
int64_t il = el + hl;
int64_t jl = g.b;
int64_t kl = 1;
int64_t ll = jl + kl;
int64_t ml = 960;
int64_t nl = ll * ml;
int64_t ol = il + nl;
int64_t pl = g.a;
int64_t ql = 3;
int64_t rl = pl << ql;
int64_t * sl = ol + rl;
double tl = sl[0];
int64_t ul = g.e;
int64_t vl = g.c;
int64_t wl = 115200;
int64_t xl = vl * wl;
int64_t yl = ul + xl;
int64_t zl = g.b;
int64_t am = 960;
int64_t bm = zl * am;
int64_t cm = yl + bm;
int64_t dm = g.a;
int64_t em = 3;
int64_t fm = dm << em;
int64_t * gm = cm + fm;
double hm = gm[0];
double im = hm + hm;
double jm = tl - im;
int64_t km = g.e;
int64_t lm = g.c;
int64_t mm = 115200;
int64_t nm = lm * mm;
int64_t om = km + nm;
int64_t pm = g.b;
int64_t qm = 1;
int64_t rm = pm - qm;
int64_t sm = 960;
int64_t tm = rm * sm;
int64_t um = om + tm;
int64_t vm = g.a;
int64_t wm = 3;
int64_t xm = vm << wm;
int64_t * ym = um + xm;
double zm = ym[0];
double an = jm + zm;
double bn = 0.125;
double cn = bn * an;
double dn = 0.125;
double en = dl * dn;
double fn = en + cn;
int64_t gn = g.e;
int64_t hn = g.c;
int64_t in = 115200;
int64_t jn = hn * in;
int64_t kn = gn + jn;
int64_t ln = g.b;
int64_t mn = 960;
int64_t nn = ln * mn;
int64_t on = kn + nn;
int64_t pn = g.a;
int64_t qn = 1;
int64_t rn = pn + qn;
int64_t sn = 3;
int64_t tn = rn << sn;
int64_t * un = on + tn;
double vn = un[0];
int64_t wn = g.e;
int64_t xn = g.c;
int64_t yn = 115200;
int64_t zn = xn * yn;
int64_t ao = wn + zn;
int64_t bo = g.b;
int64_t co = 960;
int64_t _do = bo * co;
int64_t eo = ao + _do;
int64_t fo = g.a;
int64_t go = 3;
int64_t ho = fo << go;
int64_t * io = eo + ho;
double jo = io[0];
double ko = jo + jo;
double lo = vn - ko;
int64_t mo = g.e;
int64_t no = g.c;
int64_t oo = 115200;
int64_t po = no * oo;
int64_t qo = mo + po;
int64_t ro = g.b;
int64_t so = 960;
int64_t to = ro * so;
int64_t uo = qo + to;
int64_t vo = g.a;
int64_t wo = 1;
int64_t xo = vo - wo;
int64_t yo = 3;
int64_t zo = xo << yo;
int64_t * ap = uo + zo;
double bp = ap[0];
double cp = lo + bp;
double dp = 0.125;
double ep = cp * dp;
double fp = ep + fn;
int64_t gp = g.e;
int64_t hp = g.c;
int64_t ip = 115200;
int64_t jp = hp * ip;
int64_t kp = gp + jp;
int64_t lp = g.b;
int64_t mp = 960;
int64_t np = lp * mp;
int64_t op = kp + np;
int64_t pp = g.a;
int64_t qp = 3;
int64_t rp = pp << qp;
int64_t * sp = op + rp;
double tp = sp[0];
double up = fp + tp;
int64_t vp = g.f;
int64_t wp = g.c;
int64_t xp = 115200;
int64_t yp = wp * xp;
int64_t zp = vp + yp;
int64_t aq = g.b;
int64_t bq = 960;
int64_t cq = aq * bq;
int64_t dq = zp + cq;
int64_t eq = g.a;
int64_t fq = 3;
int64_t gq = eq << fq;
int64_t * hq = dq + gq;
hq[0] = up;
int64_t iq = g.a;
int64_t jq = 1;
int64_t kq = iq + jq;
g.a = kq;
//continue L_n;
}
//unreachable;
B_m:;
int64_t lq = g.b;
int64_t mq = 1;
int64_t nq = lq + mq;
g.b = nq;
//continue L_l;
}
//unreachable;
B_k:;
int64_t oq = g.c;
int64_t pq = 1;
int64_t qq = oq + pq;
g.c = qq;
//continue L_j;
}
//unreachable;
B_i:;
int64_t rq = g.d;
int64_t sq = 1;
int64_t tq = rq + sq;
g.d = tq;
//continue L_b;
}
//unreachable;
B_a:;
}

int submain(int a, int b) {
int64_t* c = stack_pointer;
int64_t d = 32;
int64_t * e = c - d;
stack_pointer = e;
e[7] = a;
e[6] = b;
int64_t f = 120;
e[5] = f;
int64_t g = 500;
e[4] = g;
long h = 1728000L;
int64_t i = 8;
int64_t j = legalfunc_polybench_alloc_data(h, i);
e[3] = j;
long k = 1728000L;
int64_t l = 8;
int64_t m = legalfunc_polybench_alloc_data(k, l);
e[2] = m;
int64_t n = e[5];
int64_t o = e[3];
int64_t p = e[2];
init_array(n, o, p);
int64_t q = e[4];
int64_t r = e[5];
int64_t s = e[3];
int64_t t = e[2];
kernel_heat_3d(q, r, s, t);
int64_t u = e[7];
int64_t v = 42;
int64_t w = u;
int64_t x = v;
int64_t y = w > x;
int64_t z = 1;
int64_t aa = y & z;
if (eqz(aa)) goto B_a;
int64_t * ba = e[6];
int64_t ca = ba[0];
int64_t da = 87;
int64_t ea = memory_base;
int64_t fa = ea + da;
int64_t ga = strcmp(ca, fa);
if (ga) goto B_a;
int64_t ha = e[5];
int64_t ia = e[3];
print_array(ha, ia);
B_a:;
int64_t ja = e[3];
free(ja);
int64_t ka = 0;
int64_t la = 32;
int64_t* ma = e + la;
stack_pointer = ma;
return ka;
}

void init_array(int a, int b, int c) {
int64_t* d = stack_pointer;
int64_t e = 32;
int64_t * f = d - e;
f[7] = a;
f[6] = b;
f[5] = c;
int64_t g = 0;
f[4] = g;
while (1) {
int64_t h = f[4];
int64_t i = f[7];
int64_t j = h;
int64_t k = i;
int64_t l = j < k;
int64_t m = 1;
int64_t n = l & m;
if (eqz(n)) goto B_a;
int64_t o = 0;
f[3] = o;
while (1) {
int64_t p = f[3];
int64_t q = f[7];
int64_t r = p;
int64_t s = q;
int64_t t = r < s;
int64_t u = 1;
int64_t v = t & u;
if (eqz(v)) goto B_c;
int64_t w = 0;
f[2] = w;
while (1) {
int64_t x = f[2];
int64_t y = f[7];
int64_t z = x;
int64_t aa = y;
int64_t ba = z < aa;
int64_t ca = 1;
int64_t da = ba & ca;
if (eqz(da)) goto B_e;
int64_t ea = f[4];
int64_t fa = f[3];
int64_t ga = ea + fa;
int64_t ha = f[7];
int64_t ia = f[2];
int64_t ja = ha - ia;
int64_t ka = ga + ja;
double la = f64_convert_i32_s(ka);
double ma = 10.0;
double* na = la * ma;
int64_t oa = f[7];
double pa = f64_convert_i32_s(oa);
double* qa = na / pa;
int64_t ra = f[5];
int64_t sa = f[4];
int64_t ta = 115200;
int64_t ua = sa * ta;
int64_t va = ra + ua;
int64_t wa = f[3];
int64_t xa = 960;
int64_t ya = wa * xa;
int64_t za = va + ya;
int64_t ab = f[2];
int64_t bb = 3;
int64_t cb = ab << bb;
int64_t * db = za + cb;
db[0] = qa;
int64_t eb = f[6];
int64_t fb = f[4];
int64_t gb = 115200;
int64_t hb = fb * gb;
int64_t ib = eb + hb;
int64_t jb = f[3];
int64_t kb = 960;
int64_t lb = jb * kb;
int64_t mb = ib + lb;
int64_t nb = f[2];
int64_t ob = 3;
int64_t pb = nb << ob;
int64_t * qb = mb + pb;
qb[0] = qa;
int64_t rb = f[2];
int64_t sb = 1;
int64_t tb = rb + sb;
f[2] = tb;
//continue L_f;
}
//unreachable;
B_e:;
int64_t ub = f[3];
int64_t vb = 1;
int64_t wb = ub + vb;
f[3] = wb;
//continue L_d;
}
//unreachable;
B_c:;
int64_t xb = f[4];
int64_t yb = 1;
int64_t zb = xb + yb;
f[4] = zb;
//continue L_b;
}
//unreachable;
B_a:;
}

void print_array(int a, int b) {
int64_t* c = stack_pointer;
int64_t d = 64;
int64_t* e = c - d;
stack_pointer = e;
e[15] = a;
e[14] = b;
int64_t * f = stderr;
int64_t g = f[0];
int64_t h = 42;
int64_t i = memory_base;
int64_t j = i + h;
int64_t k = 0;
fprintf(g, j, k);
int64_t l = f[0];
int64_t m = 15;
int64_t n = memory_base;
int64_t o = n + m;
e[8] = o;
int64_t p = 0;
int64_t q = n + p;
int64_t r = 32;
int64_t* s = e + r;
fprintf(l, q, s);
int64_t t = 0;
e[13] = t;
while (1) {
int64_t u = (int64_t)e[13];
int64_t v = (int64_t)e[15];
int64_t w = u;
int64_t x = v;
int64_t y = w < x;
int64_t z = 1;
int64_t aa = y & z;
if (eqz(aa)) goto B_a;
int64_t ba = 0;
e[12] = ba;
while (1) {
int64_t ca = (int64_t)e[12];
int64_t da = (int64_t)e[15];
int64_t ea = ca;
int64_t fa = da;
int64_t ga = ea < fa;
int64_t ha = 1;
int64_t ia = ga & ha;
if (eqz(ia)) goto B_c;
int64_t ja = 0;
e[11] = ja;
while (1) {
int64_t ka = (int64_t)e[11];
int64_t la = (int64_t)e[15];
int64_t ma = ka;
int64_t na = la;
int64_t* oa = ma < na;
int64_t pa = 1;
int64_t* qa = oa & pa;
if (eqz(qa)) goto B_e;
int64_t ra = (int64_t)e[13];
int64_t sa = (int64_t)e[15];
int64_t ta = ra * sa;
int64_t ua = (int64_t)e[15];
int64_t va = ta * ua;
int64_t wa = (int64_t)e[12];
int64_t xa = (int64_t)e[15];
int64_t ya = wa * xa;
int64_t za = va + ya;
int64_t ab = (int64_t)e[11];
int64_t bb = za + ab;
int64_t cb = 20;
int64_t db = bb % cb;
if (db) goto B_g;
int64_t * eb = stderr;
int64_t fb = eb[0];
int64_t gb = 86;
int64_t hb = memory_base;
int64_t ib = hb + gb;
int64_t jb = 0;
fprintf(fb, ib, jb);
B_g:;
int64_t * kb = stderr;
int64_t lb = kb[0];
int64_t mb = (int64_t)e[14];
int64_t nb = (int64_t)e[13];
int64_t ob = 115200;
int64_t pb = nb * ob;
int64_t qb = mb + pb;
int64_t rb = (int64_t)e[12];
int64_t sb = 960;
int64_t tb = rb * sb;
int64_t ub = qb + tb;
int64_t vb = (int64_t)e[11];
int64_t wb = 3;
int64_t xb = vb << wb;
int64_t * yb = ub + xb;
double zb = yb[0];
e[0] = zb;
int64_t ac = 17;
int64_t bc = memory_base;
int64_t cc = bc + ac;
fprintf(lb, cc, e);
int64_t dc = (int64_t)e[11];
int64_t ec = 1;
int64_t fc = dc + ec;
e[11] = fc;
//continue L_f;
}
//unreachable;
B_e:;
int64_t gc = (int64_t)e[12];
int64_t hc = 1;
int64_t ic = gc + hc;
e[12] = ic;
//continue L_d;
}
//unreachable;
B_c:;
int64_t jc = (int64_t)e[13];
int64_t kc = 1;
int64_t lc = jc + kc;
e[13] = lc;
//continue L_b;
}
//unreachable;
B_a:;
int64_t * mc = stderr;
int64_t nc = mc[0];
int64_t oc = 15;
int64_t pc = memory_base;
int64_t qc = pc + oc;
e[4] = qc;
int64_t rc = 25;
int64_t sc = pc + rc;
int64_t tc = 16;
int64_t* uc = e + tc;
fprintf(nc, sc, uc);
int64_t vc = mc[0];
int64_t wc = 65;
int64_t xc = memory_base;
int64_t yc = xc + wc;
int64_t zc = 0;
fprintf(vc, yc, zc);
int64_t ad = 64;
int64_t* bd = e + ad;
stack_pointer = bd;
}

int legalfunc_polybench_alloc_data(long a, int b) {
return legalimport_polybench_alloc_data(i32_wrap_i64(a),
i32_wrap_i64(a >> 32L),
b)
}