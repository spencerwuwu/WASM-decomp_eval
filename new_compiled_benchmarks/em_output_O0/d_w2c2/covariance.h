#ifndef covariance_H
#define covariance_H

#include "w2c2_base.h"

typedef struct covarianceInstance {
wasmMemory*env__memory;
U32*env_____stack_pointer;
U32*env_____memory_base;
U32*env_____table_base;
U32*GOTX2Emem__stderr;
} covarianceInstance;

U32 env__strcmp(struct covarianceInstance*,U32,U32);

void env__free(struct covarianceInstance*,U32);

U32 env__fprintf(struct covarianceInstance*,U32,U32,U32);

U32 env__polybench_alloc_data(struct covarianceInstance*,U32,U32,U32);

void f4(covarianceInstance*);

void f5(covarianceInstance*);

void f6(covarianceInstance*,U32,U32,F64,U32,U32,U32);

U32 f7(covarianceInstance*,U32,U32);

void f8(covarianceInstance*,U32,U32,U32,U32);

void f9(covarianceInstance*,U32,U32);

U32 f10(covarianceInstance*,U64,U32);

void covariance____wasm_call_ctors(covarianceInstance*i);

void covariance____wasm_apply_data_relocs(covarianceInstance*i);

void covariance_kernel_covariance(covarianceInstance*i,U32 l0,U32 l1,F64 l2,U32 l3,U32 l4,U32 l5);

U32 covariance_submain(covarianceInstance*i,U32 l0,U32 l1);

void covarianceInstantiate(covarianceInstance* instance, void* resolve(const char* module, const char* name));

void covarianceFreeInstance(covarianceInstance* instance);

#endif /* covariance_H */

