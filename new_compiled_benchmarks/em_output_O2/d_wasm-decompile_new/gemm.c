

extern int64_t stack_pointer;
extern int64_t memory_base;
extern int64_t stderr;

data ()rodata(offset memory_base) =
"begin ()dump %s\00C\00%0.2lf \00\0aend ()dump %s\0a\00==BEGIN DUMP_ARR"
"AYS==\0a\00==END DUMP_ARRAYS==\0a\00";













void wasm_call_ctors() {
no;
}

void wasm_apply_data_relocs() {
no;
}

void kernel_gemm(int a, int b, int c, double d, double e, int f, int g, int h) {
int64_t k;
int64_t * l;
int64_t * i;
int64_t u;
int64_t * n;
int64_t j;
int64_t * m;
int64_t v;
int64_t o;
if (a > 0) {
int64_t r = b & -2;
int64_t s = b & 1;
int64_t t = b & -4;
int64_t q = b & 3;
while (1) {
u = b <= 0;
if (u) goto B_c;
o = 0;
j = 0;
n = 0;
if (b > 3) {
while (1) {
i = f + k * 8800;
m = i + (l = j << 3);
m[0] = m[0] * e;
m = i + (l | 8);
m[0] = m[0] * e;
m = i + (l | 16);
m[0] = m[0] * e;
i = i + (l | 24);
i[0] = i[0] * e;
j = j + 4;
n = n + 4;
if (n != t) //continue L_e;
}
}
if (eqz(q)) goto B_c;
while (1) {
i = f + k * 8800 + (j << 3);
i[0] = i[0] * e;
j = j + 1;
o = o + 1;
if (o != q) //continue L_f;
}
B_c:;
int64_t p = 0;
if (c > 0) {
while (1) {
if (u) goto B_i;
l = g + k * 9600 + (p << 3);
j = 0;
o = 0;
if (b != 1) {
while (1) {
n = f + k * 8800;
m = n + (i = j << 3);
m[0] = l[0] * d * ((v = h + p * 8800) + (double)i)[0] + m[0];
n = n + (i = i | 8);
n[0] = l[0] * d * (i + (double)v)[0] + n[0];
j = j + 2;
o = o + 2;
if (o != r) //continue L_k;
}
}
if (eqz(s)) goto B_i;
j = j << 3;
i = j + f + k * 8800;
i[0] = l[0] * d * (h + p * 8800 + (double)j)[0] + i[0];
B_i:;
p = p + 1;
if (p != c) //continue L_h;
}
}
k = k + 1;
if (k != a) //continue L_b;
}
}
}

int submain(int a, int_ptr b) {
int64_t d;
int64_t * e;
int64_t f;
int64_t * c;
int64_t * h;
int64_t j;
int64_t n;
int64_t* g = stack_pointer - 48;
stack_pointer = g;
int64_t i = legalfunc_polybench_alloc_data(1100000L, 8);
int64_t l = legalfunc_polybench_alloc_data(1200000L, 8);
int64_t m = legalfunc_polybench_alloc_data(1320000L, 8);
while (1) {
c = 0;
while (1) {
f = i + d * 8800;
f[c] = f64_convert_i32_s((c * d | 1) % 1000) / 1000.0;
f[e = c | (double)1] = f64_convert_i32_s((d * e + 1) % 1000) / 1000.0;
c = c + 2;
if (c != 1100) //continue L_b;
}
d = d + 1;
if (d != 1000) //continue L_a;
}
d = 0;
while (1) {
c = 0;
while (1) {
f = l + d * 9600;
f[c] = f64_convert_i32_s((e = c | 1) * d % 1200) / 1200.0;
f[e] = f64_convert_i32_s((c = c + 2) * d % 1200) / 1200.0;
if (c != 1200) //continue L_d;
}
d = d + 1;
if (d != 1000) //continue L_c;
}
d = 0;
while (1) {
c = 0;
while (1) {
e = m + d * 8800;
(e + (h = c << (double)3))[0] =
f64_convert_i32_s((f = c + 2) * d % 1100) / 1100.0;
(e + (h | (double)8))[0] =
f64_convert_i32_s((c + 3) * d % 1100) / 1100.0;
c = f;
if (c != 1100) //continue L_f;
}
d = d + 1;
if (d != 1200) //continue L_e;
}
while (1) {
int64_t k = 0;
d = 0;
while (1) {
f = i + j * 8800;
c = f + (d << 3);
c[0] = c[0] * 1.2;
e = c + 8;
e[0] = e[0] * 1.2;
e = c + 16;
e[0] = e[0] * 1.2;
e = c + 24;
e[0] = e[0] * 1.2;
c = c + 32;
c[0] = c[0] * 1.2;
d = d + 5;
if (d != 1100) //continue L_h;
}
while (1) {
e = l + j * 9600 + (k << 3);
c = 0;
while (1) {
h = f + (d = c << 3);
h[0] = e[0] * 1.5 * ((n = m + k * 8800) + (double)d)[0] + h[0];
h = f + (d = d | 8);
h[0] = e[0] * 1.5 * (d + (double)n)[0] + h[0];
c = c + 2;
if (c != 1100) //continue L_j;
}
k = k + 1;
if (k != 1200) //continue L_i;
}
j = j + 1;
if (j != 1000) //continue L_g;
}
if (a < 43) goto B_k;
if (ubyte))((b[0])[0] goto B_k;
c = memory_base;
fwrite(c + 42, 22, 1, f = (int))stderr[0];
g[8] = c + 15;
fiprintf(f, c, g + 32);
d = 0;
while (1) {
e = d * 1000;
c = 0;
while (1) {
if (eqz((c + e) % 20)) { fputc(10, f) }
g[2] = (i + d * (double)8800)[c];
small_fprintf(f, memory_base + 17, g + 16);
c = c + 1;
if (c != 1100) //continue L_m;
}
d = d + 1;
if (d != 1000) //continue L_l;
}
g[0] = (c = memory_base) + 15;
fiprintf(f, c + 25, g);
fwrite(c + 65, 22, 1, f);
B_k:;
free(i);
free(l);
free(m);
stack_pointer = g + 48;
return 0;
}

int legalfunc_polybench_alloc_data(long a, int b) {
return legalimport_polybench_alloc_data(i32_wrap_i64(a),
i32_wrap_i64(a >> 32L),
b)
}