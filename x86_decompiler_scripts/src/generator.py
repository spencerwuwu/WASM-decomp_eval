...
from src import replacer, modifier, Config
...
def compile_single_file(file_path, debug="", do_print=False):
...
def decompile_single_ff(file_path, generated_file_path='', func_names=''):
    ...
    if os.path.isdir(file_path):
        ...
        if Config.JEB3_test:
            ...
            status, output = IDA_decompile.decompile(file_path, generated_file_path)
        elif Config.R2_test:
...
def recompile_ff(decompiled_file='', func_names=[],
                 keep_func_decl_unchanged=1,
                 try_second_time=1):
    decompiled_code = replacer.read_file(decompiled_file)
    if Config.RetDec_test:
        decompiled_code = modifier.RetDec_modifier_before(decompiled_code)
        decompiled_code = modifier.RetDec_modifier_after(decompiled_code)
    elif Config.R2_test:
        decompiled_code = modifier.R2_modifier_before(decompiled_code)
        decompiled_code = modifier.R2_modifier_after(decompiled_code)
    with open(decompiled_file.replace(".c", "_new.c"), "w") as fd:
        fd.write(decompiled_code)
